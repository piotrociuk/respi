...
uint8_t spi_rx[10];
uint8_t spi_tx[10];
volatile uint8_t t_wdech = 1; //[s]
uint16_t objetosc_zad;
uint16_t cisnienie_zad;
uint16_t przeplyw_zad;
uint16_t czas_wdechu;
uint16_t czas_wydechu;
uint16_t PEEP;
pid_t pid1;
int pid_pwm;
int pwm = 200;
...
//cisnienie
	...
	adc_voltage[0] = (3.3 * adc_srednia[0]) / 1024.0;
...
//pid_init(&pid1, pid_p, pid_i, pid_d, 100);
//	pid_init(&pid1, 0, 0, 0, 1000);
//HAL_GPIO_WritePin(ZAWOR_GPIO_Port, ZAWOR_Pin, 1);
...
//
//		//wydech
//		__HAL_TIM_SET_COMPARE(&htim10, TIM_CHANNEL_1, objetosc_zad);
//		HAL_GPIO_WritePin(ZAWOR_GPIO_Port, ZAWOR_Pin, 0);
//		HAL_Delay(600);
//		__HAL_TIM_SET_COMPARE(&htim10, TIM_CHANNEL_1, objetosc_zad * 4);
//		HAL_Delay(1200);
//		__HAL_TIM_SET_COMPARE(&htim10, TIM_CHANNEL_1, objetosc_zad / 2);
//		HAL_Delay(200);
